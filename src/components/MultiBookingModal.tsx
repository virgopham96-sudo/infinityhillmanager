...
export default function MultiBookingModal({ rooms, onClose, onUpdateRooms }: MultiBookingModalProps) {
  const defaultCheckIn = set(new Date(), { hours: 14, minutes: 0, seconds: 0, milliseconds: 0 });
  const defaultCheckOut = addDays(set(new Date(), { hours: 12, minutes: 0, seconds: 0, milliseconds: 0 }), 1);
  ...
  const handleReserve = () => {
    ...
    if (selectedRoomIds.length === 0) return alert('Vui lòng chọn ít nhất 1 phòng');

    const inDate = new Date(checkIn);
    const outDate = new Date(checkOut);
    if (inDate >= outDate) {
      return alert("Thời gian Check-in phải trước Check-out.");
    }
    ...
    if (conflictingRooms.length > 0) {
      return alert(`Các phòng sau bị trùng lịch đặt trước: ${conflictingRooms.join(', ')}`);
    }

    const depositPerRoom = selectedRoomIds.length > 0 ? totalDeposit / selectedRoomIds.length : 0;
  ...
  const totalExpectedPrice = selectedRoomIds.reduce((total, id) => {
    ...
    return total + calculateTotalPrice(checkIn, checkOut, room.weekdayPrice, room.weekendPrice);
  }, 0);
        ...
        <div className="p-6 overflow-y-auto flex-1 flex flex-col gap-6">
          ...
          <div>
            <div className="flex justify-between items-end mb-3">
              ...
              <button 
                onClick={() => setSelectedRoomIds(selectedRoomIds.length === availableRooms.length ? [] : availableRooms.map(r => r.id))}
                className="text-sm text-blue-600 hover:text-blue-700 font-medium"
              >
                {selectedRoomIds.length === availableRooms.length ? 'Bỏ chọn tất cả' : 'Chọn tất cả'}
              </button>
            </div>
              ...
              {availableRooms.map(room => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-xs font-medium opacity-70">{room.type}</span>
                  </button>
                );
              })}
          ...
          <div className="mt-auto p-4 bg-emerald-50 rounded-lg border border-emerald-100 flex flex-col gap-2">
            ...
            {totalDeposit > 0 && (
              <div className="flex items-center justify-between text-amber-600 border-t border-emerald-200/50 pt-2 mt-1">
                ...
                <span className="font-semibold text-base">-{formatCurrency(totalDeposit)}</span>
              </div>
            )}
